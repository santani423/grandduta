...
class FPDF_AutoWrapTableTidakDiangkut extends FPDF {
private $data = array();
...
$this->Ln(10);
$this->SetFont("", "B", 15);
$this->SetX($left); $this->Cell(0, 14, 'DAFTAR SAMPAH TIDAK DIANGKUT', 0, 1,'C');
$this->SetX($left); $this->Cell(0, 14, 'PERIODE TANGGAL : ' .  date('d') . ' ' . $this->tampil_bulan(date('n')) . ' ' . date('Y'), 0, 1,'C');
$this->Ln(10);
	...
	$this->SetX($left += 70); $this->Cell(80, $h, 'TUNGGAKAN', 1, 0, 'C',true);
	$this->SetX($left += 80); $this->Cell(110, $h, 'TOTAL TAGIHAN', 1, 0, 'C',true);
	$this->Ln(20);
	...
	(
		...
		$baris->nokav,				
		$baris->jmltunggakan,
		$this->separator($baris->jmltagihan)
			)
	); 
	}
